...
import pool.config.ThriftConnectionPoolConfig.ServiceOrder;
import pool.config.ThriftConnectionPoolConfig.ThriftServiceType;
import pool.config.ThriftServerInfo;
import pool.connection.DefaultThriftConnection;
import pool.connection.MulitServiceThriftConnecion;
import pool.connection.ThriftConnection;
	...
	private ThriftConnection<T> obtainRawInternalConnection(ThriftServerInfo serverInfo)
		...
		if (this.thriftServiceType == ThriftServiceType.SINGLE_INTERFACE) {
			connection = new DefaultThriftConnection<>(serverInfo.getHost(), serverInfo.getPort(),
					this.connectionTimeOut, this.config.getThriftProtocol(),this.config.getThriftTransport(), this.config.getClientClass());
		} else {
	...
	public synchronized void shutdown() {
		if (!this.poolShuttingDown) {
			...
			this.asyncExecutor.shutdownNow();
                    ...
                    try {
                        this.asyncExecutor.awaitTermination(5, TimeUnit.SECONDS); // do nothing
                    } catch (InterruptedException ex) {
                        java.util.logging.Logger.getLogger(ThriftConnectionPool.class.getName()).log(Level.SEVERE, null, ex);
                    }
