...
import Strategy.DatabaseMappingMissing.DatabaseMappingMissingLevelDB;
import Configuration.ConfigOfSystem;
import DBConstantString.DBString;
import Object.LogLogin;
import Object.LogPayment;
import Strategy.DatabaseRecomment.DatabaseRecommentHashTable;
...
public class DatabasePerformLevelDB implements DatabasePerformStrategy{
    ...
    private String databaseRecommentName = DBString.databaseRecommentName;
    ...
    public void accessToDatabase(LogLogin logLogin) throws Exception {
        ...
        try {
            ...
            if (infoSession == null) {
                ...
                dbMapping = factory.open(new File(DBString.databaseMappingName), options);
                try{
                ...
                for (int i = 0; i < jsonArr.size(); i++) {
                    ...
                    String gameID = jsonObj.get(DBString.gameIDString).getAsString();
                    listGameNeedAddPoint.add(gameID);
                    if (gameID.equals(logLogin.getGameID())) {
                        long point = jsonObj.get(DBString.pointString).getAsLong();
                        long amount = jsonObj.get(DBString.amountString).getAsLong();
                        long week = jsonObj.get(DBString.weekString).getAsLong();

                        jsonObj.remove(DBString.pointString);
                        jsonObj.remove(DBString.amountString);
                        jsonObj.remove(DBString.weekString);

                        point += (long) point * Math.pow(2.0, week - this.getWeek());
                        amount += 0;
                        week = this.getWeek();

                        jsonObj.addProperty(DBString.pointString, point);
                        jsonObj.addProperty(DBString.amountString, amount);
                        jsonObj.addProperty(DBString.weekString, week);
                    }
        ...
        try {
            ...
            if (session == null) {
                ...
                for (int i = 0; i < jsonArr.size(); i++) {
                    ...
                    if (jsonObj.get(DBString.gameIDString).getAsString().equals(logPayment.getGameID())) {
    ...
    private JsonArray getListTopGame(String session) throws IOException{
        this.dbSession = factory.open(new File(DBString.databaseName), options);
        try{
            ...
            for (int i = 0; i < jsonArr.size(); i++){
                jsonObj = (JsonObject) jsonArr.get(i);
                JsonPrimitive element = new JsonPrimitive(jsonObj.get(DBString.gameIDString).getAsString());
...
//"]";
    ...
    private JsonObject createNewElement(String userID,String gameID){
        ...
        jsonObj.addProperty(DBString.pointString, ConfigOfSystem.pointPerLogin);
        jsonObj.addProperty(DBString.userIDString, userID);
        jsonObj.addProperty(DBString.gameIDString, gameID);
        jsonObj.addProperty(DBString.amountString, ConfigOfSystem.initAmount);
        jsonObj.addProperty(DBString.loginTimesString, ConfigOfSystem.initLoginTimes);
        jsonObj.addProperty(DBString.weekString, this.getWeek());
        return jsonObj;
    }
}
